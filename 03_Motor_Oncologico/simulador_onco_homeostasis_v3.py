...
class SimuladorTratamiento:
    ...
    def ejecutar_simulacion(self, cohorte="C", mutacion_mct2=False):
        ...
        for t in tiempo:
            ...
            if t < t_metabolico:
                ...
                # Si MCT2 está sobreexpresado, rescata el pHi evitando que descienda a 5.75
                pHi_minimo = 5.75 + 0.85 * (1 - 1.0 / tumor.mct2_expresion)
                decay_pHi = (7.20 - pHi_minimo) * (1 - math.exp(-0.4 * (t - t_metabolico)))
                tumor.pHi = max(pHi_minimo, 7.20 - decay_pHi)
                
                # El eflujo de protones residual por MCT2 impide que el pHe intersticial se limpie por completo a 7.35
                pHe_maximo = 7.35 - 0.75 * (1 - 1.0 / tumor.mct2_expresion)
                lavado_pHe = (pHe_maximo - 6.20) * (1 - math.exp(-0.25 * (t - t_metabolico)))
                tumor.pHe = min(pHe_maximo, 6.20 + lavado_pHe)
                
                # Colapso mitigado de ATP: MCT2 permite retener cierta respiración oxidativa/glucolítica residual
                atp_minimo = 30.0 + 770.0 * (1 - 1.0 / tumor.mct2_expresion)
                atp_drop = (10000.0 - atp_minimo) * (1 - math.exp(-0.35 * (t - t_metabolico)))
                tumor.atp_nivel = max(atp_minimo, 10000.0 - atp_drop)
                
                # Recuperación pasiva de la eficiencia de linfocitos CD8+ TILs (atenuada por acidez estromal residual de MCT2)
                if tumor.pHe > 7.0:
