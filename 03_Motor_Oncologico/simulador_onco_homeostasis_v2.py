...
class CelulaHumana:
    """Estado biofisico ionico/genomico (modelo Capa B)."""
...
if __name__ == "__main__":
    print("=== Homeostasis oncologica v2.2 (modelo) ===\n")

    sana = CelulaHumana(tipo_celular="Sana")
    regulador_sana = ReguladorRestricciones(sana)

    print("[SANA] Acortamiento telomerico (80 pb/div, 50 ciclos):")
    for _ in range(50):
    ...
    print(f"  generacion={sana.divisiones}  telomeros={sana.telomeros} pb  "
          f"(umbral={UMBRAL_SENESCENCIA_TELOMEROS})")
    print(f"  V_m={pot_sana:.2f} mV  viabilidad={viab_sana * 100:.1f}%")
