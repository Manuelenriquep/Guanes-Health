...
class TestSimuladorOncoHepaticoCoupled(unittest.TestCase):
    ...
    def test_coupled_cointervention_clearance(self):
        """
        Prueba 4: Curación Completa Co-Intervención (Escenario 4).
        Verifica que la administración conjunta de Myrcludex B (10 nM) y el bloqueo selectivo de IL-6/STAT3
        (beta_pd_l1 = 0.1) bajo el protocolo de la Cohorte C logre neutralizar por completo el escape
        del tumor (viabilidad terminal -> 0.00%) manteniendo la carga viral controlada (< 25 viriones)
        sin inducir deplesión de glutatión o toxicidad por colestasis.
        """
        ...
        self.assertAlmostEqual(res["viabilidad_tumor"][-1], 0.00, places=2)
        self.assertLess(res["carga_viral"][-1], 25.0, "La carga viral debe estar fuertemente deprimida (< 25 viriones)")
        self.assertLess(res["pd_l1_tumor"][-1], 150.0, "PD-L1 debe estar muy por debajo del umbral de saturación de 150x")
        self.assertGreater(res["gsh"][-1], 5.0, "El pool de GSH debe permanecer saludable (> 5 mM)")
